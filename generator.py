...
class Generator:
    ...
    def generate_list(self, n):
        """
        Метод для генерации n-го числа последовательности
        Нужен просто для упрощения понимания всего кода
        """
        row = list(self.base_num_row)
        if n < 65:
            return row[::n]
        else:
            for ind in range(65,n,1):
                row.append((row[ind-18] + row[ind-65]) % self.m)
        return row
